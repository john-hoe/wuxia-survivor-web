 ...
 * - 渲染尺寸 = 960×K × 540×K（桌面 K≤2，受限触屏设备 K≤1.5，见 main.ts）：
...
/** 渲染倍率：桌面上限 2，受限触屏设备上限 1.5；DPR=1 时与旧行为一致。 */
...
function resolveResolutionScale(): number {
  ...
  const constrainedDevice = typeof window !== "undefined"
    && (window.matchMedia?.("(pointer: coarse)").matches
      || "ontouchstart" in window
      || Math.min(window.innerWidth, window.innerHeight) <= 540);
  // Mobile fill-rate and memory are the limiting resources. A 1.5× cap keeps
  // text sharper than DPR1 while reducing RGBA attachment pixels by 43.75%
  // versus DPR2. Desktop Retina retains DPR2.
  return Math.min(dpr, constrainedDevice ? 1.5 : 2);
}
